...
docker run --rm -d -p 8044:8080 --name "$name" -v $(pwd)/config:/config -e SWAGGER_FILE=/config/openapi.yml swaggerapi/swagger-editor

wait_container_to_be_running "$name" & sleep 2

# open swagger-ui in browser
# open http://localhost:8044 -- ONLY WORKS ON OSX

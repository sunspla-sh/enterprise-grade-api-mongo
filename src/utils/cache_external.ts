...
class CacheExternal {
  ...
  private static _instance: CacheExternal
  ...
  public static getInstance(): CacheExternal {

    if(!CacheExternal._instance){

      CacheExternal._instance = new CacheExternal();

    }

    return CacheExternal._instance;
  ...
  public setProp(key: string, value: string, expireAfter: number): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const result = this._client!.setex(key, expireAfter, value, err => {
  ...
  public getProp(key: string): Promise<string|undefined> {
    ...
    return new Promise((resolve, reject) => {
      ...
      const result = this._client!.get(key, (error, result) => {
